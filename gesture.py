...
def prepare_training_data(gesture_data):
    """Prepare training data with proper train/test split and augmentation"""
    X, y = [], []
    ...
    # Apply different augmentation to train and test
    X_train = augment_data(X_train)
    X_test = add_minimal_noise(X_test)  # Only slight noise to test
...
def augment_data(samples):
    """Add realistic variations to training data"""
    augmented = []
    for sample in samples:
        # Original sample
        augmented.append(sample)
        
        # Add noise (10-20% of value range)
        ...
        augmented.append(sample * scale)
        
        # Axis swapping (simulate different viewpoints)
        if len(sample) > 20:  # Only if we have enough landmarks
            swapped = sample.copy()
            swapped[::3] = sample[1::3]  # Swap x and y coordinates
            swapped[1::3] = sample[::3]
            augmented.append(swapped)
    
    return np.array(augmented)

def add_minimal_noise(samples):
    """Add slight noise to test data (much less than training)"""
    noisy_samples = []
    for sample in samples:
        noise = np.random.normal(0, 0.05, sample.shape) * (np.max(sample) - np.min(sample))
...
def train_gesture_model(gesture_data, model_type, epochs=30, incremental=True):
    """Train selected model type with improved regularization and evaluation"""
    if not gesture_data or len(gesture_data) < 2:
        st.error("Need at least 2 different gestures to train")
        return None
        
    # Prepare data with proper stratification and augmentation
    X, y = [], []
    gesture_names = list(gesture_data.keys())
    min_samples = min(len(samples) for samples in gesture_data.values())
    ...
    # Split data with stratification BEFORE augmentation
    X_train, X_test, y_train, y_test = train_test_split(
        X, y_encoded, 
        test_size=0.3,  # Increased test size
    ...
    if model_type in ["CNN", "LSTM"]:
        ...
        for sample, label in zip(X_train, y_train):
            ...
            # Add noisy version (10-20% random noise)
            noise = np.random.normal(0, 0.15, sample.shape) * (np.max(sample) - np.min(sample))
            ...
            # Add axis-swapped version (simulates different viewpoints)
            if len(sample) > 20:  # Only if we have enough landmarks
                swapped = sample.copy()
                swapped[::3] = sample[1::3]  # Swap x and y coordinates
                swapped[1::3] = sample[::3]
        ...
        # Add minimal noise to test set (less than training)
        noisy_test = []
        for sample in X_test:
            noise = np.random.normal(0, 0.05, sample.shape) * (np.max(sample) - np.min(sample))
            noisy_test.append(sample + noise)
        X_test = np.array(noisy_test)
    ...
    # Model-specific processing with improved regularization
    if model_type == "SVM":
        if can_update:
            st.info("New data detected - retraining SVM from scratch")
        
        # Use reduced C parameter to prevent overfitting
        model = make_pipeline(
            StandardScaler(),
            SVC(kernel='rbf', C=1.0, gamma='scale', probability=True)  # Reduced from C=10
        )
            ...
            if model_type == "CNN":
                model = Sequential([
                    ...
                    Conv1D(128, 3, activation='relu',
                          kernel_regularizer=tf.keras.regularizers.l2(0.01)),
                    Dropout(0.5),
                    GlobalAveragePooling1D(),  # Changed from Flatten
        ...
        plot_confusion_matrix(np.argmax(y_test_cat, axis=1), y_pred, gesture_names)
...
def predict_gesture(model, model_type, landmarks, confidence_threshold=0.7):
    ...
    try:
        ...
        return None, 0
def evaluate_model(model, model_type, X_test, y_test, gesture_names):
    ...
    low_confidence = np.mean(max_probs < 0.7)  # % of uncertain predictions
    ...
    return {
        ...
        'low_confidence_pct': low_confidence
    }
        ...
        with st.container():
            ...
            with st.expander("🗂️ Gesture Administration", expanded=False):
                with st.container():
                    ...
                    st.markdown("</div>", unsafe_allow_html=True)
def show_train_page():
    """Professional model training interface with enhanced visualization"""
    ...
    .metric-card {{
        background: {'#2a3f5f' if st.session_state.dark_mode else '#f8f9fa'};
        border-radius: 8px;
        padding: 1rem;
        margin-bottom: 1rem;
    }}
    ...
    st.caption("Train and optimize gesture recognition models with different architectures")
        ...
        with st.container():
            ...
            <div class="training-card">
                <div style="display: flex; align-items: center; gap: 1rem;">
                    ...
                    <div>
                        <h4 style="margin: 0; color: {'#ffffff' if dark_mode else '#2a3f5f'};">Insufficient Gesture Data</h4>
                        <p style="margin: 0; color: {'#b0b0b0' if dark_mode else '#666'};">Record at least 2 different gestures to begin training</p>
                    </div>
                </div>
                <div style="margin-top: 1rem;">
            """, unsafe_allow_html=True)
            
            if st.button("Go to Recording Studio", key="go_to_record"):
        ...
        with st.container():
            ...
            <div class="training-card">
                <div style="display: flex; align-items: center; gap: 1rem;">
                    <div style="font-size: 2rem;">⚠️</div>
                    <div>
                        <h4 style="margin: 0; color: {'#ffffff' if dark_mode else '#2a3f5f'};">{len(unmapped_gestures)} Unmapped Gestures</h4>
                        <p style="margin: 0; color: {'#b0b0b0' if dark_mode else '#666'};">Map gestures to actions for full functionality</p>
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    with st.container():
        st.markdown("""
        <div class="training-card">
            <h3 style="color: {'#ffffff' if st.session_state.dark_mode else '#2a3f5f'};">1. Model Configuration</h3>
        """, unsafe_allow_html=True)
        
        tab1, tab2 = st.tabs(["Architecture Selection", "Training Parameters"])
    ...
    with st.container():
        st.markdown("""
        <div class="training-card">
            <h3 style="color: {'#ffffff' if st.session_state.dark_mode else '#2a3f5f'};">2. Training Progress</h3>
        """, unsafe_allow_html=True)
        ...
        with col1:
            with st.container():
                ...
                <div class="metric-card">
                    <h4 style="margin: 0 0 0.5rem 0; color: {'#ffffff' if st.session_state.dark_mode else '#2a3f5f'};">Live Metrics</h4>
                """, unsafe_allow_html=True)
            ...
            with st.container():
                ...
                <div class="training-card">
                    <div style="display: flex; align-items: center; gap: 1rem;">
                        <div style="font-size: 2rem;">⚠️</div>
                        <div>
                            <h4 style="margin: 0; color: {'#ffffff' if st.session_state.dark_mode else '#2a3f5f'};">Low Sample Warning</h4>
                            <p style="margin: 0; color: {'#b0b0b0' if st.session_state.dark_mode else '#666'};">Some gestures have <20 samples - accuracy may suffer</p>
                        </div>
                    </div>
                    <div style="margin-top: 1rem;">
                """, unsafe_allow_html=True)
                
                if not st.checkbox("Proceed with training anyway (not recommended)"):
                    st.markdown("</div></div>", unsafe_allow_html=True)
                    return
                
                st.markdown("</div></div>", unsafe_allow_html=True)

        with st.spinner(f"Initializing {MODEL_OPTIONS[model_type]} training session..."):
            try:
                # Prepare data
                X, y = [], []
...
def show_comparison_page():
    ...
    if any(m in selected_models for m in ["CNN", "LSTM"]):
